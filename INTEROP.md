...
How **dnsaudit** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `dnsaudit` lives in the **Network / recon** cluster.
...
| from | relation | to |
|---|---|---|
| `dnsaudit` | JSON findings compose with | [`nmapparse`](https://github.com/cognis-digital/nmapparse), [`pcapview`](https://github.com/cognis-digital/pcapview), [`dnsrecon`](https://github.com/cognis-digital/dnsrecon), [`subhunt`](https://github.com/cognis-digital/subhunt) |
| `dnsaudit` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `dnsaudit` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `dnsaudit` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the network / recon cluster.** `dnsaudit` output feeds the next tool:
```bash
dnsaudit ... --format json > out.json          # this tool's findings (see `dnsaudit --help`)
...
dnsaudit ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
dnsaudit ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
dnsaudit ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `dnsaudit`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `dnsaudit` slots
