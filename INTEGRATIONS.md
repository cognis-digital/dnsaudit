...
**dnsaudit** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `dnsaudit` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
dnsaudit ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
dnsaudit ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
dnsaudit ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
dnsaudit ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `dnsaudit` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="dnsaudit") for rec in records]   # records = your JSON output
