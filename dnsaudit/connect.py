"""Native cognis-connect emit for dnsaudit — forward findings to any platform.

Maps dnsaudit's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    dnsaudit ... --format json | dnsaudit-emit --to stix
    dnsaudit-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = f"{rec.get('type', '')} {rec.get('name', '')}"
        out['severity'] = 'info'
        if rec.get('type') == 'spoof':
            out['severity'] = 'high'
        elif rec.get('type') in ['nxdomain', 'cname', 'txt']:
            out['severity'] = 'medium'
        out['type'] = rec.get('type', '')
        out['description'] = f"{rec.get('name', '')} {rec.get('value', '')}"
        out['tags'] = [rec.get('type', ''), 'dns']
        out['ipv4'] = rec.get('ipv4')
        out['domain'] = rec.get('name')
        out['url'] = rec.get('url')
